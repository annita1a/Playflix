  ...
  <b-row class="streamit-wraper-table2">
      ...
      <b-card no-body>
        <b-card-header>
          <h2 class="episode-playlist-title wp-heading-inline">
            <span class="position-relative"> {{$t('addpage.add-new-video-playlist')}} </span>
          </h2>
        </b-card-header>
        <b-card-body>
          <form>
            <div class="form-group">
              <InputField id="genre-name" :label="$t('addpage.playlist_name')" type="text" :placeholder="$t('addpage.enter-name-video-playlist')" :is-required="true" />
            </div>
            <div class="form-group">
              <InputField id="genre-slug" :label="$t('addpage.playlist_slug')" type="text" :placeholder="$t('addpage.enter-slug-video-playlist')" :is-required="true" />
            </div>

            <div class="d-flex align-items-center justify-content-end mt-4">
              <button class="btn btn-primary custom-add-button">{{$t('addpage.add-video-playlist')}}</button>
            </div>
          </form>
        </b-card-body>
      </b-card>
    ...
    <b-col lg="8">
      <b-card no-body>
        <div class="pb-3">
          <b-card-header class="d-flex justify-content-between align-items-center">
            <h2 class="episode-playlist-title wp-heading-inline">
              <span class="position-relative"> {{$t('addpage.video-playlists')}} </span>
            </h2>
          </b-card-header>
          <b-card-body>
              ...
              <table id="seasonTable" ref="tableRef1" class="data-tables data-table-one table custom-table movie_table dataTable no-footer">
                <thead>
                  <tr class="text-uppercase">
                    ...
                    <th class="sorting" tabindex="0" aria-controls="seasonTable" rowspan="1" colspan="1" aria-label="Playlist Name: activate to sort column ascending">Playlist Name</th>
                    <th class="sorting" tabindex="0" aria-controls="seasonTable" rowspan="1" colspan="1" aria-label="Playlist Slug: activate to sort column ascending">Playlist Slug</th>
      ...
      <b-offcanvas v-model="offcanvasEnd1" class="offcanvas-width-80" header-close-class="me-0" :placement="themeSchemeDirection !== 'ltr' ? 'start' : 'end'">
        <template #header>
          <h2 class="episode-playlist-title wp-heading-inline">
            <span class="position-relative"> {{$t('addpage.update-video-playlist')}} </span>
          </h2>
        ...
        <b-form class="section-form">
          ...
          <div class="form-group">
            <InputField id="playlist-slug" :label="$t('addpage.playlist_slug')" type="text" :placeholder="$t('addpage.enter-slug-playlist')" :is-required="true" />
          </div>
        </b-form>
        <template #footer>
...
const tableData = ref([
  { title: 'My Favorite Videos', slug: 'myfavoritevideos' },
  { title: 'Knowledge Hub', slug: 'knowledgehub' },
  { title: 'Love Stories', slug: 'lovestories' },
]);
