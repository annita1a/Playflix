      ...
      <b-card no-body>
        ...
        <div class="border rounded py-4">
          <div class="custom-table-effect table-responsive mb-0">
            <table id="datatable" ref="tableRef" class="table mb-0 dataTable no-footer" data-toggle="data-table">
            </table>
          </div>
        </div>
      </b-card>
...
const columns = ref([
  {
    ...
    data: 'user',
    render: function (row) {
      return `<div class="d-flex align-items-center"><img class="rounded img-fluid avatar-60 me-3" src="${generateImgPath(row.img)}" alt="img"/><div class="media-support-info"><h5 class="mb-0">${row.name}</h5><p class="mb-0">${row.desc}</p></div></div>`;
    },
  },
  {
    title: 'Contact',
