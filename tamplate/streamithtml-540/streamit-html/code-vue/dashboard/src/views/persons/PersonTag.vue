  ...
  <b-row class="streamit-wraper-table2">
    ...
    <b-col lg="8">
      ...
      <b-offcanvas v-model="offcanvasEnd1" class="offcanvas-width-80" footer-hide header-close-class="me-0" :placement="themeSchemeDirection !== 'ltr' ? 'start' : 'end'">
        <template #header>
          <h2 class="episode-playlist-title wp-heading-inline">
            <span class="position-relative"> {{$t('addpage.update-tag')}} </span>
          </h2>
        ...
        <b-form class="section-form custom-from-section-bottom">
          <div class="form-group">
            <InputField id="genre-name" :label="$t('addpage.tag-name')" model-value="Producer" type="text" :placeholder="$t('addpage.enter-title-tag')" :is-required="true" />
          </div>
          <div class="form-group">
            <InputField id="genre-slug" :label="$t('addpage.tag-slug')" model-value="Producer" type="text" :placeholder="$t('addpage.enter-slug-tag')" :is-required="true" />
          </div>
    ...
    render: function () {
      return `
           <div class="d-flex align-items-center list-user-action gap-2">
              <button type="button" class="btn btn-sm btn-icon btn-success-subtle rounded"  data-bs-toggle="tooltip" data-bs-placement="top" title="Edit"  data-table="action" data-id="0" data-method="edit" aria-label="Edit" data-bs-original-title="Edit">
