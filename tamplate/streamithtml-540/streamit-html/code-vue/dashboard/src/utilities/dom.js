...
export const setAttr = function (elems, object) {
  console.log('setAttr called with:', { elems, object })
  ...
  console.log('Elements to update:', _newElem.length || 1)
  
  _.forEach(_newElem, function (elem) {
    const oldValue = elem.getAttribute(object.prop)
    console.log(`Updating ${object.prop} from "${oldValue}" to "${object.value}" on:`, elem)
    elem.setAttribute(object.prop, object.value)
    
    // Verify the change
    const newValue = elem.getAttribute(object.prop)
    console.log(`Attribute ${object.prop} is now:`, newValue)
    
    // For RTL/LTR changes, also update the style
    if (object.prop === 'dir') {
      elem.style.direction = object.value
      console.log('Updated direction style to:', object.value)
    }
  })
}
