      ...
      <div class="slider">
        <div class="slider-flex position-relative">
          <div class="slider--col position-relative">
            <div
              class="vertical-slider-prev swiper-button" tabindex="0" role="button" aria-label="Previous slide"
            ...
            <div class="slider-thumbs" data-swiper="slider-thumbs">
              <div class="swiper-container" data-swiper="slider-thumbs-inner">
                <swiper
                  v-if="render" ref="swiperRef" :dir="rtl" :space-between="24" :slides-per-view="3" :navigation="{
                  nextEl: '.vertical-slider-next',
                  prevEl: '.vertical-slider-prev',
                  }" :loop="true" :thumbs="{ swiper: thumbsSwiper }" :centered-slides="true" :watch-slides-progress="true" :modules="modules"
                  :direction="'vertical'" class="swiper-wrapper top-ten-slider-nav" @swiper="onSwiper">
                  <SwiperSlide v-for="(data, index) in sliderData" :key="data"  tag="li" class="swiper-slide swiper-bg" @click="() => slideToCenter(index)">
                    <div v-if="HomePageSlider !== true" class="block-images position-relative ">
                      <div class="img-box slider--image">
                        <img :src="data.image" class="w-100 rounded-3" alt="img" loading="lazy">
                      </div>
                ...
                <div class="slider-next btn-verticle"><i class="ph ph-caret-down"></i></div>
              </div>
            </div>
            <div
              class="vertical-slider-next swiper-button" tabindex="0" role="button" aria-label="Next slide"
              aria-controls="swiper-wrapper-b130b4e10e1468117"><i class="ph ph-caret-down"></i></div>
          </div>
          <div class="slider-images" data-swiper="slider-images">
            <swiper
              :loop="true" :space-between="10" :modules="modules" :effect="'fade'"
              ...
              <SwiperSlide v-for="data in sliderData" :key="data" tag="li" class="swiper-slide">
                <div class="slider--image block-images"><img :src="data.image" loading="lazy" alt="img" /></div>
                  ...
                  <div class="block-description">
                    <ul class="ps-0 mb-2 pb-1 list-inline d-flex flex-wrap align-items-center movie-tag justify-content-center justify-content-lg-start genres-list gap-1 gap-sm-0">
                      <li class="position-relative text-capitalize font-size-14 letter-spacing-1">
                        <router-link :to="{ name: 'landing-page.view-all' }" class="text-decoration-none">{{ $t(data.geners) }}</router-link>
                      </li>
                      <li class="position-relative text-capitalize font-size-14 letter-spacing-1">
                        <router-link :to="{ name: 'landing-page.view-all' }" class="text-decoration-none">{{ $t(data.geners1) }}</router-link>
                      </li>
                      <li class="position-relative text-capitalize font-size-14 letter-spacing-1">
                        <router-link :to="{ name: 'landing-page.view-all' }" class="text-decoration-none">{{ $t(data.geners2) }}</router-link>
                      </li>
                      <li class="position-relative text-capitalize font-size-14 letter-spacing-1">
                        <router-link :to="{ name: 'landing-page.view-all' }" class="text-decoration-none">{{ $t(data.geners3) }}</router-link>
                      </li>
                    </ul>
                    <h2 class="iq-title m-0 line-count-2">
                      <router-link :to="{ name: 'landing-page.tv-show-detail' }">{{ $t(data.title) }}</router-link>
                    </h2>
                    <div class="d-flex align-items-center gap-3 py-2 justify-content-center justify-content-lg-start flex-wrap">
                      ...
                      <div class="d-flex align-items-center gap-1">
                        ...
                        <img class="imdb-img" alt="imdb-logo" :src="generateImgPath('/assets/images/pages/imdb-logo.svg')" />
                      </div>
                      <div class="d-flex align-items-center gap-1">
                    ...
                    <custom-button :button-title="$t('streamButtons.play_now')" :link="{ path: '/' }"></custom-button>
                  </div>
                </div>
              </SwiperSlide>
