          ...
          <div class="modal-header pb-0">
            <template v-if="istextureTitle">
              <h3 class="text-uppercase m-0 texture-text texture-text-modal fw-bold">
                {{ resolveText(movieName) }}
              </h3>
            </template>
            <template v-else>
              <h5 class="css_prefix-title text-capitalize line-count-1">
                <a href="#" class="color-inherit">{{ resolveText(movieName) }}</a>
              </h5>
            ...
            <div v-if="isTag" class="d-flex align-items-baseline flex-wrap gap-2 mt-3">
              ...
              >
                <li v-for="tag in tags" :key="tag">
                  <a href="#" class="position-relative">{{ resolveText(tag) }}</a>
                </li>
              </ul>
            </div>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body pt-1">
            ...
            <div class="d-flex align-items-baseline flex-wrap gap-2 mt-md-1 mt-2">
              <h6 class="m-0">{{ resolveText('streamPages.geners') }}:</h6>
              <ul class="p-0 mb-0 list-inline d-flex flex-wrap movie-tag">
                <li v-for="genre in genres" :key="genre" class="trending-list">
                  <a href="#">{{ resolveText(genre) }}</a>
                </li>
              </ul>
            </div>
            ...
            <div class="d-flex align-items-baseline row-gap-1 column-gap-2 mt-4">
              <h6 class="m-0">{{ resolveText('streamPages.cast') }}:</h6>
              <ul
              ...
              >
                <li v-for="cast in castList" :key="cast">
                  <a href="#" class="color-inherit">{{ resolveText(cast) }}</a>
                </li>
              </ul>
            </div>

            <div class="d-flex align-items-baseline row-gap-1 column-gap-2 mt-4">
              <h6 class="m-0">{{ resolveText('sectionTitle.crew') }}:</h6>
              <ul
                class="list-inline m-0 p-0 d-flex align-items-center flex-wrap row-gap-1 column-gap-2 cast-crew-list"
              >
                <li v-for="crew in crewList" :key="crew">
                  <a href="#" class="color-inherit">{{ resolveText(crew) }}</a>
                </li>
              </ul>
            </div>
          </div>
...
function resolveText(value) {
  ...
  // If it's a valid i18n key, translate it; else show as plain text
  return te(value) ? t(value) : value
}

defineProps({
  isEpisodepage: { type: Boolean, default: false },
  istextureTitle: { type: Boolean, default: false },
  movieName: { type: String, default: '' },
  movieReleased: { type: [String, Number], default: '' },
  isNotClockduration: { type: Boolean, default: false },
  movieDuration: { type: String, default: '' },
  movieViews: { type: [String, Number], default: '' },
  isNotimdbRating: { type: Boolean, default: false },
  imdbRating: { type: [String, Number], default: '' },
  ...
  isLanguadge: { type: Boolean, default: false },
  language: { type: String, default: 'English (UK)' },
  description: { type: String, default: 'streamMovies.game_of_heros_desc' },
  moviedurettion: { type: Boolean, default: false },
