...
let removeTvshowGenre = document.getElementById("remove_tvshow_genre_thumbnail_buttons");
if (removeTvshowGenre) {
  removeTvshowGenre.addEventListener("click", function () {
    const element = document.getElementById(
      "remove_tvshow_genre_thumbnail_buttons"
    );
    if (element) {
      element.remove(); // This removes the entire element and its content from the DOM
    }

    // document.getElementById('tvshow_genre_thumbnail_previews').innerHTML = ""
    const image = document.getElementById("tvshow_genre_thumbnail_previews");
    if (image) {
      image.src = ""; // Removes the image source but keeps the <img> tag intact
    }

    const buttonContainer = document.querySelector(".edit-button");

    const removeButton = document.getElementById(
      "remove_tvshow_genre_thumbnail_button"
    );
    // let remove_class  = document.getElementById()
    console.log(buttonContainer);
    buttonContainer.innerHTML = "";
    buttonContainer.innerHTML = template;

    toggleRemoveButton(removeButton, buttonContainer);
  });
}

// Improved, defensive handler for legacy single remove button
// Finds the nearest `.streamit-upload` root (if present) and performs
// the same cleanup while avoiding null errors and ensuring toggleRemoveButton
// receives the proper `root` context.
(function () {
  ...
  removeBtn.addEventListener("click", function (e) {
    ...
    // Prefer the closest widget root; fall back to document
    const root = removeBtn.closest(".streamit-upload") || document;

    // Remove the buttons element if it exists inside the root
    ...
    // Clear the preview image (if an <img>) or clear innerHTML if not
    const image = root.querySelector("#tvshow_genre_thumbnail_previews");
    if (image) {
    ...
    // Restore the button container template if present
    const buttonContainer = root.querySelector(".edit-button") ||
      document.querySelector(".edit-button");
    if (buttonContainer) {
      buttonContainer.innerHTML = template;
    }

    // Find the singular remove button (if any) within the same root
    const removeButtonSingular = root.querySelector(
      "#remove_tvshow_genre_thumbnail_button"
    );

    // Pass the root so toggleRemoveButton can scope its queries correctly
    toggleRemoveButton(removeButtonSingular, buttonContainer, root);
  });
})();
