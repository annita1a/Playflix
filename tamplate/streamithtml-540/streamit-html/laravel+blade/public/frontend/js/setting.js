...
(function (window) {
  ...
  // DEBUG: Apply theme from sessionStorage on page load
  document.addEventListener('DOMContentLoaded', function () {
    try {
      var theme = null;
      // Try to get theme from sessionStorage
      if (sessionStorage.getItem('streamit')) {
        var streamitData = JSON.parse(sessionStorage.getItem('streamit'));
        if (streamitData && streamitData.setting && streamitData.setting.theme_style_appearance) {
          theme = streamitData.setting.theme_style_appearance.value;
        }
      }
      // Fallback to default
      if (!theme) theme = 'dark';
      // DEBUG LOG
      // console.log('[IQSetting] Theme from sessionStorage:', theme);
      // Apply theme to <html>
      document.documentElement.setAttribute('data-bs-theme', theme);
    } catch (e) {
      // console.error('[IQSetting] Error reading theme from sessionStorage:', e);
    }
  });
  ...
  function defaultSettingOption() {
    return {
      ...
      theme_style_appearance: {
        ...
        value: "dark", // Changed from ["dark"] to "dark"
      },
    };
  }
  ...
  this.IQSetting = function (opt) {
    ...
    // Load stored options and merge with defaults to ensure all keys exist
    const storedOptions = this.getOption(this.options.storeKey);
    if (storedOptions && storedOptions !== 'none') {
      // Deep merge defaults and storedOptions
      this.options = IQUtils.mergeDeep(defaults, storedOptions);
    }
  ...
  IQSetting.prototype.__attributeUpdate__ = function (
    ...
    const obj = this.getSettingKey(key);
    if (!obj) {
      console.error(`[IQSetting] __attributeUpdate__: key '${key}' not found in options.setting.`);
      return;
    }
    if (pair.value !== null) {
