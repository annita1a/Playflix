...
export const settingSlice = createSlice({
  ...
  reducers: {
    ...
    theme_language: (state, action) => {
      if (typeof action.payload !== typeof undefined) {
        state.setting.theme_language.value = action.payload;
      }
      updateHtmlAttr({
        prop: "lang",
        value: state.setting.theme_language.value,
      });

      // Keep direction in sync with language: Arabic -> rtl, others -> ltr
      try {
        const lang = state.setting.theme_language.value;
        const dir = String(lang) === "ar" ? "rtl" : "ltr";
        state.setting.theme_scheme_direction.value = dir;
        // update html dir attribute immediately
        updateHtmlAttr({ prop: 'dir', value: state.setting.theme_scheme_direction.value });
      } catch (e) {
        // ignore
      }
