...
const ShopCategorySidebar = memo((props: ShopCategorySidebarProps) => {
  ...
  const handleSliderChange = (values: any) => {
    ...
    const roundedValues = values.map((value: any) => Math.round(value));
    setSliderValues(roundedValues[0]);
  };

  const product_tag = [
    "streamShopTag.camera",
    "streamShopTag.costume",
    "streamShopTag.lighting",
    "streamShopTag.production",
    "streamShopTag.studios",
  ];
  ...
  return (
    <Fragment>
            ...
            <Collapse in={open2}>
              <ul className="list-unstyled ps-2 mt-3">
                <li>
                  <Form.Check // prettier-ignore
                    type="checkbox"
                    label={t("merchandise.ghost_spider")}
                  />
                </li>
              </ul>
            </Collapse>
      ...
      <div className="shop-box border-bottom-0 ">
        <h4 className="title-product-sidbar text-uppercase">
          {t("streamShop.product")}
        </h4>
        <ul className="list-unstyled m-0 p-0 shop-product">{props.children}</ul>
      </div>
    </Fragment>
  );
});
