<template>
  <div>
    ...
    <section v-if="activeSection === 'movie'" class="section-padding">
      <b-container fluid>
        <div class="d-flex align-items-center justify-content-between mb-4">
          <h4 class="main-title text-capitalize mb-0">{{$t('streamPages.movie_tags')}}</h4>
        </div>
        ...
        <div v-if="movieLimit < movieTags.length" class="text-center mt-4">
          <div class="iq-button">
            <a href="javascript:void(0)" class="btn btn-primary position-relative custom-button load-more-btn rounded-3" @click="movieLimit += loadStep">
    ...
    <section v-else-if="activeSection === 'tvshow'" class="section-padding">
      <b-container fluid>
        <div class="d-flex align-items-center justify-content-between mb-4">
          <h4 class="main-title text-capitalize mb-0">{{$t('streamPages.tv_show_tags')}}</h4>
        </div>
        <b-row class="gy-4 row-cols-2 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 row-cols-xl-5 data-listing">
        ...
        <div v-if="tvShowLimit < tvShowTags.length" class="text-center mt-4">
          <div class="iq-button">
            <a href="javascript:void(0)" class="btn btn-primary position-relative custom-button load-more-btn rounded-3" @click="tvShowLimit += loadStep">
              <span class="button-text">{{$t('streamButtons.load_more')}}</span>
            </a>
          </div>
        </div>
    ...
    <section v-else-if="activeSection === 'video'" class="section-padding">
      <b-container fluid>
        <div class="d-flex align-items-center justify-content-between mb-4">
          <h4 class="main-title text-capitalize mb-0">{{$t('streamPages.video_tags')}}</h4>
        </div>
        <b-row class="gy-4 row-cols-2 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 row-cols-xl-5 data-listing">
        ...
        <div v-if="videoLimit < videoTags.length" class="text-center mt-4">
          <div class="iq-button">
            <a href="javascript:void(0)" class="btn btn-primary position-relative custom-button load-more-btn rounded-3" @click="videoLimit += loadStep">
              <span class="button-text">{{$t('streamButtons.load_more')}}</span>
            </a>
          </div>
        </div>
