<template>
  <div>
          ...
          <div class="image-wrap">
            <nuxt-link :to="{name: 'merchandise-product-detail'}">
              <div class="product-image">
                <img :src="generateImgPath(`/assets/images/shop/product/${productImg}`)" class="img-fluid w-100 rounded-3" :alt="`productImg-${id}`" loading="lazy" />
              </div>
            </nuxt-link>
        ...
        <b-col md="8">
          <div class="product-caption">
            <h4 class="product__title mb-2">
              <nuxt-link :to="{ name: 'merchandise-product-detail' }" class="title-link">{{ shopTitle }}</nuxt-link>
            </h4>
            ...
            <ul class="iq-button-holder list-inline d-flex align-items-center flex-wrap gap-3">
              ...
              <li>
                <a href="#" class="add_to_wishlist wishlist-btn d-flex align-items-center justify-content-center border-0 rounded-3" @click="wishlistSwal">
    ...
    <div v-else class="product-block">
      ...
      <div class="product-caption">
        <nuxt-link :to="{name: 'merchandise-product-detail'}" class="title-link text-primary">{{ shopTitle }}</nuxt-link>
        <div class="price-detail">
  ...
  <div id="woosq-popup" class="modal fade woosq-popup" tabindex="-1" aria-modal="true" role="dialog" data-bs-backdrop="static" data-bs-keyboard="false" :class="{ show: modal }" :style="{ display: modal ? 'block' : 'none' }">
    <div class="modal-dialog modal-dialog-centered positon-relative">
      <div class="modal-content rounded-0 border-0">
        <div class="modal-body p-0">
          <button type="button" class="btn-close position-absolute" data-bs-dismiss="modal" aria-label="Close" @click="modal = false"></button>
