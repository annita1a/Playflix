<template>
  <div>
    ...
    <div class="d-block d-lg-flex align-items-center">
      <h5
        v-if="isnotmovieTitle"
        class="css_prefix-title text-capitalize line-count-1"
      >
    ...
    <div id="readmore-wrapper" class="movie-description mt-3 mb-4">
      ...
      <div class="iq-blog-meta-cat-tag iq-blogtag readmore-tags">
        <a
          ...
          data-bs-target="#viewMoreDataModal"
    ...
    <!-- Movie Metadata -->
    <ul
      v-if="isVideoPageData"
      class="list-inline mx-0 p-0 d-flex align-items-center flex-wrap gap-3 movie-metalist"
    >
      <li>
    ...
    <ul
      v-else
      class="list-inline mb-0 mx-0 p-0 d-flex align-items-center flex-wrap gap-3 movie-metalist"
    >
      <li>
        <span class="fw-medium">{{ movieReleased }}</span>
      </li>
      ...
      <li>
        <div class="d-flex align-items-center gap-1">
          <i class="ph ph-eye"></i>
          <span>{{ viewNumber }} {{ $t(movieViews) }}</span>
        </div>
      </li>
    ...
    <div
      v-if="movieLanguage"
      class="video-language d-flex align-items-center gap-1 mt-2"
    >
      <i class="ph ph-translate"></i>
      <ul
        class="list-inline m-0 p-0 d-inline-flex align-items-center gap-3 flex-wrap"
      >
        <li>
    ...
    <div class="d-flex align-items-center flex-wrap gap-3 gap-md-4 my-5">
      <div v-if="!isNotstartWatching" class="iq-play-button iq-button">
        <nuxt-link :to="videoUrl" class="btn btn-primary w-100 rounded d-flex align-items-center justify-content-center gap-2 lh-1">
            <i
              :class="subscribeToWatch ? 'ph-fill ph-crown' : 'ph-fill ph-play'"
              class="fs-6"
            ></i>
            <span>{{
              $t(
                subscribeToWatch
                  ? "streamButtons.subscribe_watch"
      ...
      <div v-if="!isNotwatchList" class="watchlist-button-wrapper">
        <nuxt-link
          v-b-tooltip.hover
          :title="$t('streamTooltip.add_to_watchlist')"
          ...
          class="btn btn-secondary border rounded d-flex align-items-center justify-content-center gap-2 lh-1">
            ...
            <span class="fw-semibold">{{ $t("streamButtons.watch_list") }}</span>
        </nuxt-link>
      </div>

      <div class="d-flex align-items-center gap-3 flex-wrap">
        <button
          v-if="!isNotLikemovies"
          id="like-movies"
          class="action-btn btn btn-secondary border"
          @click="toggleLike"
        >
          <span id="like-movies">
            <span
              ...
              :title="$t(isLiked ? 'streamSocial.unlike' : 'streamSocial.like')"
            >
              <i
                :class="[isLiked ? 'ph-fill' : 'ph', 'ph-heart', 'heart-icon']"
              ></i>
            </span>
          </span>
        </button>
        ...
        <button
          v-if="!isNotPlaylistbtn"
          class="btn btn-secondary action-btn border"
          data-bs-toggle="modal"
    ...
    <DetailsDescriptionModal
      :istexture-title="istextureTitle"
      :movie-name="$t(movieName)"
      :movie-released="movieReleased"
      :movie-duration="movieDuration"
      :movie-views="movieViews"
      :is-not-clockduration="isNotClockduration"
      :is-notimdb-rating="isNotimdbRating"
      :imdb-rating="imdbRating"
      :is-episodepage="isEpisodepage"
    />
  </div>
</template>
