<template>
  <div class="tab-slider otthome-tab-slider">
    <div class="slider">
      <Swiper v-if="render" :dir="rtl" :watch-slides-progress="true"
        class="wrapper-class position-relative swiper swiper-card" :pagination="{
          el: '.swiper-pagination',
          clickable: true
        }" :navigation="{
          prevEl: '.joint-arrows .swiper-button-prev',
          nextEl: '.joint-arrows .swiper-button-next'
        }" :effect="'fade'" :slides-per-view="1" :modules="modules">
        <SwiperSlide v-for="(data, index) in tabSlider" :key="`slider-${index}`" tag="li"
          class="tab-slider-banner p-0 swiper-active">
          <div class="tab-slider-banner-images" :style="{ backgroundImage: `url(${data.image})` }">
                    ...
                    <div class="tab-left-details">
                      <div class="d-flex align-items-center gap-3 mb-4">
                        <a href="javascript:void(0);">
                          <img src="/assets/images/pages/trending-label.webp"
                            class="img-fluid trending-label-img rounded-3" alt="trending" loading="lazy" />
                        </a>
                        <span class="text-gold fw-bold font-size-18">
                          #{{ data.movieRank || '' }} {{ $t(data.movieTime || '') }}
                        </span>
                      </div>
                      ...
                      <ul class="d-flex align-items-center list-inline gap-2 movie-tag p-0 mt-3 mb-40">
                        <li class="font-size-18 trending-list">{{ data.movieDate || '' }}</li>
                        <li class="font-size-18">{{ data.seasonNumber || '' }} {{ $t('streamEpisode.season') }}</li>
                      </ul>
                      <custom-button :button-title="$t('streamButtons.stream_now')" :link="{ name: 'tv-shows-detail' }" />
                    </div>
                  </b-col>
                    ...
                    <div class="tab-block">
                      ...
                      <div class="tab-bottom-bordered border-0">
                        <ul class="nav nav-tabs nav-pills mb-3 overflow-x-scroll" role="tablist">
                          <li v-for="(season, seasonIndex) in data.seasons" :key="`season-${season.id}`"
                            class="nav-item" role="presentation">
                            <button class="nav-link" :class="{ active: selectedTab[index] === seasonIndex }"
                              data-bs-toggle="pill" :data-bs-target="`#season-${index}-${season.id}`" type="button"
                              role="tab" :aria-selected="selectedTab[index] === seasonIndex"
                              @click="changeTab(index, seasonIndex)">
                              {{ $t('streamEpisode.season') }} {{ season.number }}
                            </button>
                          </li>
                        </ul>
                      </div>
                      ...
                      <div class="tab-content iq-tab-fade-up">
                        <div v-for="(season, seasonIndex) in data.seasons" :id="`season-${index}-${season.id}`"
                          :key="`season-content-${season.id}`" class="tab-pane fade" :class="{
                            show: selectedTab[index] === seasonIndex,
                            active: selectedTab[index] === seasonIndex
                          }" role="tabpanel" tabindex="0">
                          <ul class="list-inline m-0 p-0">
                            <li v-for="(episode, episodeIndex) in season.episodes" :key="`episode-${episodeIndex}`"
                              class="d-flex align-items-center gap-3">
                              <div class="image-box flex-shrink-0">
                                <nuxt-link :to="{ name: 'tv-shows-detail' }">
                                  <img :src="episode.image" :alt="`episode-${episodeIndex + 1}`"
