        ...
        <ul class="list-inline mb-0">
          <li class="posted-by">
            <span class="post-date d-flex align-item-center gap-1">
              ...
              <nuxt-link :to="{ name: 'blogs-filter-type', params: { type: 'author' } }">{{ blogTag }}</nuxt-link>
            </span>
          </li>{{ '' }}
          <li class="posted-on">
          ...
          <template v-if="blogCategories && blogCategories.length">
            <li v-for="(cat, idx) in blogCategories" :key="`cat-${idx}`" class="blog-category">
              <nuxt-link :to="{ name: 'blogs-filter-type', params: { type: 'category' } }" class="pe-1">{{ $t(cat) }}</nuxt-link>
            </li>
          </template>
        </ul>
      </div>
